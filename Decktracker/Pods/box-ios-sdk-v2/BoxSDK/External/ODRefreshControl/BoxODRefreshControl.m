...
#define kBoxTotalViewHeight    400
#define kBoxOpenedViewHeight   44
#define kBoxMinTopPadding      9
#define kBoxMaxTopPadding      5
#define kBoxMinTopRadius       12.5
#define kBoxMaxTopRadius       16
#define kBoxMinBottomRadius    3
#define kBoxMaxBottomRadius    16
#define kBoxMinBottomPadding   4
#define kBoxMaxBottomPadding   6
#define kBoxMinArrowSize       2
#define kBoxMaxArrowSize       3
#define kBoxMinArrowRadius     5
#define kBoxMaxArrowRadius     7
#define kBoxMaxDistance        53

@interface BoxODRefreshControl ()
...
{
    self = [super initWithFrame:CGRectMake(0, -(kBoxTotalViewHeight + scrollView.contentInset.top), scrollView.frame.size.width, kBoxTotalViewHeight)];
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
    if ([keyPath isEqualToString:@"contentInset"]) {
        if (!_ignoreInset) {
            ...
            self.frame = CGRectMake(0, -(kBoxTotalViewHeight + self.scrollView.contentInset.top), self.scrollView.frame.size.width, kBoxTotalViewHeight);
        }
        return;
    }
    ...
    if (_refreshing) {
        if (offset != 0) {
            ...
            _shapeLayer.position = CGPointMake(0, kBoxMaxDistance + offset + kBoxOpenedViewHeight);
            [CATransaction commit];

            _activity.center = CGPointMake(floor(self.frame.size.width / 2), MIN(offset + self.frame.size.height + floor(kBoxOpenedViewHeight / 2), self.frame.size.height - kBoxOpenedViewHeight/ 2));
            ...
            if (offset < 0) {
                ...
                if (offset >= -kBoxOpenedViewHeight) {
                    if (!self.scrollView.dragging) {
                        ...
                        if (_hasSectionHeaders) {
                            [self.scrollView setContentInset:UIEdgeInsetsMake(MIN(-offset, kBoxOpenedViewHeight) + self.originalContentInset.top, self.originalContentInset.left, self.originalContentInset.bottom, self.originalContentInset.right)];
                        } else {
                            [self.scrollView setContentInset:UIEdgeInsetsMake(kBoxOpenedViewHeight + self.originalContentInset.top, self.originalContentInset.left, self.originalContentInset.bottom, self.originalContentInset.right)];
                        }
    ...
    CGFloat verticalShift = MAX(0, -((kBoxMaxTopRadius + kBoxMaxBottomRadius + kBoxMaxTopPadding + kBoxMaxBottomPadding) + offset));
    CGFloat distance = MIN(kBoxMaxDistance, fabs(verticalShift));
    CGFloat percentage = 1 - (distance / kBoxMaxDistance);
    
    CGFloat currentTopPadding = BoxLerp(kBoxMinTopPadding, kBoxMaxTopPadding, percentage);
    CGFloat currentTopRadius = BoxLerp(kBoxMinTopRadius, kBoxMaxTopRadius, percentage);
    CGFloat currentBottomRadius = BoxLerp(kBoxMinBottomRadius, kBoxMaxBottomRadius, percentage);
    CGFloat currentBottomPadding =  BoxLerp(kBoxMinBottomPadding, kBoxMaxBottomPadding, percentage);
    ...
    if (distance == 0) {
        ...
        if (percentage == 0) {
            bottomOrigin.y -= (fabs(verticalShift) - kBoxMaxDistance);
            triggered = YES;
        }
    }
    ...
    //Left curve
    CGPoint leftCp1 = CGPointMake(BoxLerp((topOrigin.x - currentTopRadius), (bottomOrigin.x - currentBottomRadius), 0.1), BoxLerp(topOrigin.y, bottomOrigin.y, 0.2));
    CGPoint leftCp2 = CGPointMake(BoxLerp((topOrigin.x - currentTopRadius), (bottomOrigin.x - currentBottomRadius), 0.9), BoxLerp(topOrigin.y, bottomOrigin.y, 0.2));
    ...
    //Right curve
    CGPoint rightCp2 = CGPointMake(BoxLerp((topOrigin.x + currentTopRadius), (bottomOrigin.x + currentBottomRadius), 0.1), BoxLerp(topOrigin.y, bottomOrigin.y, 0.2));
    CGPoint rightCp1 = CGPointMake(BoxLerp((topOrigin.x + currentTopRadius), (bottomOrigin.x + currentBottomRadius), 0.9), BoxLerp(topOrigin.y, bottomOrigin.y, 0.2));
    ...
    if (!triggered) {
        ...
        CGFloat currentArrowSize = BoxLerp(kBoxMinArrowSize, kBoxMaxArrowSize, percentage);
        CGFloat currentArrowRadius = BoxLerp(kBoxMinArrowRadius, kBoxMaxArrowRadius, percentage);
        ...
        CGFloat radius = BoxLerp(kBoxMinBottomRadius, kBoxMaxBottomRadius, 0.2);
